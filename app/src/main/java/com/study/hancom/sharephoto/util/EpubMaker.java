    ...
    /* 페이지 별로 html 생성*/
        ...
        try {
            ...
            stringBuilder.insert(stringBuilder.indexOf(OEBPS_PACKAGE_METADATA), String.format(mContext.getResources().getString(R.string.epubData_OEBPS_package_metaData),
                    fileName, author, new SimpleDateFormat("yyyy-MM-dd", java.util.Locale.getDefault()), publisher,UUID.randomUUID().toString()));
        ...
        try {
            ...
            stringBuilder.insert(stringBuilder.indexOf(HEAD), String.format(mContext.getResources().getString(R.string.epubData_OEPBS_toc_meta), UUID.randomUUID().toString()));
            for (int i = 0; i < pageCount; i++) {
